    ...
    if (!result) {
		...
		if ([status isEqualToString:@"OK"]) {
			...
			if ([status isEqualToString:@"ERROR"]) {
                ...
                NSInteger code = [errorDict[@"errorCode"] integerValue];
                NSString *errorMsg = errorDict[@"errorMessage"];
                NSError *error = [NSError errorWithDomain:errorMsg code:code userInfo:errorDict];
                [self failedWithError:error];
			}
		}
	}
